...
## 9. Research addendum (2026-09-26): all-target hits, hit-stun, and what feeds a chain

Asked by the independent check of Chapter XV (IC-1, IC-2). **FFX-2 only.** Read 2026-09-26. GameFAQs first (Bailey, 2026-09-25: "I especially like gamefaqs"); the HD Remaster is taken to behave as the PS2 original, as no source read says otherwise. Quotes are kept to a few words; the rest is paraphrase.

### 9.1 Q1. Does an all-target enemy move hit each living target exactly once?

| Question | Answer | Status |
|---|---|---|
| Single-hit all-target move (Lightfall) | **One hit on every character.** SinirothX (FAQ 31807, Nooj): "damage all characters by 5000 HP"; Split_Infinity (FAQ 25872, Nooj): "VS all chrs - 5000 universal damage"; FF Wiki *Final Fantasy X-2 enemy abilities* (rev 3998493): "Inflicts 5000 damage to the party". The move is defined per target, so no character takes it twice. | `[verified: 3 sources]` |
| A target dies partway through | **No source says it in words.** Every source defines an all-target move per character ("all characters", "the party"), so a hit belongs to its target: a target KO'd before its hit simply takes nothing, and nothing is moved onto another character. A retarget would give one character two hits of "5000 to every character", which contradicts all three. | per-target semantics `[verified: 3 sources]`; the mid-move death case itself **unsourced** (inference from the definition) |
| Multi-hit **random**-target moves | **Different, and sourced as such.** Each hit picks a random member of the target group: SinirothX writes "damage random characters 3 times" (Triple Attack and kin); FF Wiki lists "N ... attacks on random party members" for Angra Mainyu, the gems, Ultima Weapon and others; Split_Infinity: gem hits are "randomly distributed between targets in target group". KeyBlade999 (FAQ 69206, lightning mix): all three hits land on a lone enemy ("450 damage to a single enemy"), so the random pick ranges over whoever is present. | `[verified: 3 sources]` |
| Can a random hit land on a character already KO'd? | Not stated anywhere read. | **unsourced** |

**What the engine does now** (`src/battle/ffx2/resolve.ts` `targetForHit`): for `all-*` targeting it takes `living[hitIndex % living.length]` from a list re-filtered after every hit. When a target dies mid-move the list shrinks, later indices wrap onto a girl already hit, and the last girl is never hit (IC-2: Lightfall hit Yuna, Rikku KO, Yuna again, Paine never). **This contradicts the sourced per-target definition.** Random-target moves re-roll among the living on every strike, which matches the sources for the part that is sourced.

**What the engine must do to match:** take the target list **once, at the start of the action** (the living targets, or the dead for `can-target-dead`); give each of them its own hits (`ability.hits` each for a multi-strike all-target move such as Sword Dance, 2 per enemy); if one dies partway, skip its remaining hits and **do not redirect** them. Keep the per-strike random pick for `random-*` moves. Excluding the KO'd from the random pick stays an unsourced engine choice.

### 9.2 Q2. Does being hit hold, delay or reset the victim's turn? What feeds a chain?

| Question | Answer | Status |
|---|---|---|
| Is there hit-stun? | **Yes: the "stagger".** A hit makes the target play a recovery (stagger) animation; the chain window **is** that recovery. Split G0912: strike again "while target is still recovering from being hit"; KeyBlade999 (HD Remaster guide, Full Chain trophy): the enemy "is unable to recover from the first attack before the next"; FF Wiki *Chain (term)* (rev 4043750): "still staggered from the previous attack". Some units skip the stagger on some hits: SinirothX marks one scythe-armed fiend's Normal Attack 2 as "doesn't use stagger animation when attacked". A Stopped unit cannot be chained because its stagger never plays (FF Wiki *Stop (Final Fantasy X-2 status)* rev 4015588; *Final Fantasy X-2 statuses* rev 4028042). Stagger length varies by body: the Steam thread "FFX-2 99 Chain Achievement" (Ex-SOLDIER) calls Chac's stagger "rather long". | `[verified: 3 sources]`; per-unit stagger lengths **unsourced** |
| What does the stagger do to the victim's turn? | While staggered the victim **cannot start its own action**, and a long enough chain keeps it from ever acting; an attack whose animation has already begun still goes off (FF Wiki *Chain (term)*). Player reports agree: Split Boss Guide (FAQ 26832, Chac, strategy sent by "Amy"): each hit "interrupts whatever action" she was about to use; the Steam thread above: Chac "will never even get one attack in". | `[verified: 3 sources]` (unchanged from §1.7) |
| Does being hit delay the gauge itself? | **Yes, qualitatively, for any damaging hit.** Split G0905 rule 8: any attack "which deals damage will temporarily meddle with your bar filling process", fiends included. Jegged *ATB and Chain Attacks*: a chain needs the next hit to land "before the enemy's posture recovers and their ATB gauge keeps climbing", which reads as the gauge not climbing during the stagger. **How much** the fill is perturbed is not published. Split G0906 adds that a girl in Critical HP (yellow, kneeling) "requires more time to act and recover from hits". | perturbation `[verified: 2 sources]`; amount **unsourced**; Critical-HP recovery `[single source]` |
| Does a hit reset the gauge or cancel a command? | **Only for abilities that carry it.** Delay effect (DELEF) empties a set % of the ATB or CTIM bar, and a girl hit by it while choosing loses her menu (Split G1041, example with Iron Giant); Action-cancel (ACTIC) cancels a charging command and restarts the ATB (Split G1042). Both are named per ability (Bruiser Tail, Blast Punch, Huggles, Mach Fan...). **Correction to §1.1:** its "being hit while the command menu is open cancels the menu and delays the turn" comes from Split's DELEF example; the source ties it to Delay-effect attacks, not every hit. | `[verified: 2 sources]` (§1.4 table) |
| Is there a "Break" on the victim? | **No.** FFX-2's "Break" abilities (Power Break etc.) are stat downs. No source read describes a victim-side break or stun other than the stagger above, Delay effect and Action-cancel. | absence in every source read |
| Is the chain only the attacker's damage bonus? | **No.** The chain count raises the next hit's damage, **and** the same stagger stops the victim evading and starting its action. Enemies can chain the party the same way (Split G0912: enemies can do the "same chaining effect on your team"; FF Wiki: "Both the player party and the enemy party can chain attacks"). | `[verified: 2 sources]` |
| Does a **miss** count toward a chain? | **No.** A chain is built by hits that land: Split G0912, "Once you land a successful hit" and "each successive attack that connects"; KeyBlade999, "when your attacks land"; FF Wiki, "while a unit is being hit". | `[verified: 3 sources]` |
| Does an **immune** or **Invincible** hit count? | **Not stated anywhere read.** The nearest words: Split G1032 says attacks on an Invincible target "will fail"; Split G0905 rule 8 limits the gauge perturbation to attacks that deal damage; Split G1030 says Null Physical shows 'IMMUNE' "when target is hit", which is ambiguous on whether it staggers. **Our reading of GameFAQs (Split_Infinity), labelled as an estimate:** a hit that deals no damage because of Invincible, or an 'IMMUNE' result, does not stagger and does not open or extend a chain. | **unsourced**; the reading above is `[estimate: Split_Infinity wording]` and needs Bailey's word or a real-game look |

**Why a lone enemy could chain-lock a girl in our build (IC-1), and what the sources say about it.** Nooj's bar is 5,737 ticks (1.91 s) against a 6,000-tick (2 s) window, so while every Nooj action registers a hit, a lone girl never leaves her window. Two engine choices produce this, and neither is sourced: (a) the immune hit registers (above); (b) an action takes **no time** in our engine, while the sources say the actor waits out its own execution before its gauge refills (`research/ffx2-trema.md` §12.4, E4, `[verified: 2 sources]`, length unsourced). The window (2 s) and Nooj's numbers are sourced and **must not be tuned** to hide this (AGENTS.md rule 6; memory *boss-side-fix-needs-measured-options*).

### 9.3 Q3. What the engine models now, and on what

- `src/battle/ffx2/chain.ts`: `registerHit` opens a 2 s window (3 s after a crit) and returns the chain count; the window runs on the global clock (an engine choice: the stagger is an animation, so Haste and Slow on the victim do not stretch it; unsourced). `isActionLocked` = `CHAIN_LOCKS_ACTIONS && window open && not charging`: a unit already charging still fires (matches the wiki's "already begun its attack animation"). `cannotEvade` = window open. `breakChain` on KO. Sources: §1.7 (FF Wiki *Chain (term)*, Split G0912), `[verified: 2 sources]`.
- `src/battle/ffx2/active.ts` `canTakeTurn` and `engine.ts` (PR-0076 "held" command): a locked unit keeps a full gauge and waits; the gauge itself is **not** perturbed by a hit (§9.2 row 3 says it should be, amount unsourced).
- `src/battle/ffx2/resolve.ts`, per hit: the hit check comes first (a miss by evasion returns before the chain, which matches §9.2), then `registerHit` **before** `computeDamage`, so an **immune** result still opens or extends the window and emits a `chain` event (IC-1; unsourced either way, see §9.2). Restoratives (`heals` / `healing`) never register: an engine ruling recorded in the comment there (a Cure is not a hit), consistent with every source defining the chain by attacks.
- `research/ffx2-trema.md` §12.4 (E4): action time is sourced as non-zero and unmeasured; the engine uses zero.

**What the engine must do to match (for the driver; a change in difficulty is measured and put to Bailey once, never tuned):**

1. **IC-2 (fix, sourced):** snapshot the targets of an all-target move at the start; each target gets its own hits; a target that dies is skipped, never replaced. Random-target moves keep their per-hit roll.
2. **IC-1 (needs Bailey's word):** decide whether an immune or Invincible hit registers a chain. The GameFAQs reading (Split_Infinity: attacks on Invincible "will fail"; only damaging attacks meddle with the bar) says **no**. To implement it, decide immunity before `registerHit` and register only hits that land and deal damage (Sentinel's 1 HP still counts as damage). Measure Chapter XV with and without it before asking.
3. **Recorded, not a fix for now:** a damaging hit perturbs the victim's gauge (2 sources, amount unsourced), and actions take time (E4, 2 sources, length unsourced). Both need a number nobody publishes; they stay open gaps, not guesses.

### 9.4 Sources for this section (read 2026-09-26)

- **Split_Infinity (Damir Kolar)**, *Guide and Walkthrough* ("Cool Rikku", 2011), GameFAQs FAQ 25872: G0905 rule 8, G0906, G0912, G0913, G1030, G1032, G1041, G1042, and the Nooj boss block. https://gamefaqs.gamespot.com/ps2/562386-final-fantasy-x-2/faqs/25872
- **Split_Infinity**, *Boss Guide* ("Leifang", 2008), GameFAQs FAQ 26832: Chac (Amy's Thief strategy), Paragon. https://gamefaqs.gamespot.com/ps2/562386-final-fantasy-x-2/faqs/26832
- **SinirothX**, *Enemy Encyclopedia* v1.3 (2007), GameFAQs FAQ 31807: "Note on Chain Attack", Nooj, the "stagger animation" notes, the random multi-hit attacks. https://gamefaqs.gamespot.com/ps2/562386-final-fantasy-x-2/faqs/31807
- **KeyBlade999**, *FFX-2 FAQ/Walkthrough* v1.60 (2015, HD Remaster), GameFAQs FAQ 69206: Chac chain strategy, Full Chain trophy, mix hit distribution. https://gamefaqs.gamespot.com/ps2/562386-final-fantasy-x-2/faqs/69206 (its chain multiplier "0.5 × (X − 1) + 1.45" repeats the SinirothX typo; §1.7 stands)
- Also searched, nothing new on these questions: Ryu_Kaze *Ultimania Translations* (FAQ 42601), Blackestmage (28684), bremen (26991: chaining is attacking before an enemy "can get a move inbetween"), NightMare185 *Via Infinito* (27609).
- **FF Wiki** (MediaWiki API, 2026-09-26): *Chain (term)* rev 4043750; *Stop (Final Fantasy X-2 status)* rev 4015588; *Final Fantasy X-2 statuses* rev 4028042 (Invincible "nullifies all damage"); *Final Fantasy X-2 enemy abilities* rev 3998493; *Active Time Battle* rev 3995539.
- **Jegged.com**, *Final Fantasy X-2 Tips: ATB and Chain Attacks* (undated): https://jegged.com/Games/Final-Fantasy-X-2/Tips-and-Tricks/ATB-and-Chain-Attacks.html
- **Steam Community**, "FFX-2 99 Chain Achievement" (FFX/X-2 HD Remaster discussions, post of 2016-06-01): https://steamcommunity.com/app/359870/discussions/0/357287935541731043/
