...
2. **Final Fantasy Wiki (finalfantasy.fandom.com)** — per-dressphere per-level stat tables (values match SinirothX's algorithms), ability AP/MP tables, status descriptions.
3. **Split Infinity, "Guide and Walkthrough" (GameFAQs FAQ 25872)** — the only complete per-ability tagging of charge-time / recovery-time / Protect / Shell / crit-capable / range, plus complete item and Garment Grid tables.
4. StarNeptune "Dressphere Ability FAQ", misc. board threads — corroboration only.
...
| **Stat scaling** | **The more nodes the Garment Grid has, the higher the main unit's HP, MP, Str, Mag, Def and MDef after transforming.** Only the *main* part is affected. This is a real, exploitable mechanic — a 6-node grid (First Steps, Black Tabard, Blood of the Beast, Chaos Maelstrom, Pride of the Sword, Samurai's Honor, Tricks of the Trade, White Signet) gives the strongest SDSP. |
...
**Sinistral Wing** — commands: *Sinistral Arts · Stamina · Mettle · Reboot*. Buff wing — the "Feather" abilities apply party buffs (e.g. **Steel Feather**, 12 MP, `STR Up +2 levels` to the whole target party), plus Diamond, White, Buckle, Cloudy, Pointed, **Pumice** (Haste) and **Ma'at's** Feather, with Stamina / Mettle / Reboot as HP, MP and revival commands. `[single source]`

**Dextral Wing** — commands: *Dextral Arts · Stamina · Mettle · Reboot*. Debuff wing — **Venom Wing** (Poison), **Blind Wing** (Darkness), **Mute Wing** (Silence), **Rock Wing** (Petrification), **Lazy Wing** (Slow), **Violent Wing** (Berserk), **Still Wing** (Stop), **Crazy Wing** (Confusion), plus the same Stamina / Mettle / Reboot commands. `[single source]`
...
**AP** is awarded *during* battle rather than on this screen: every action a girl takes while wearing a dressphere feeds AP into the ability she is currently learning, and **killing an enemy awards AP to all three girls**, not only the killer `[single source]`. AP is also blocked by **Pointless**.
...
**Multipliers to model**: Double EXP (Lady Luck, 80 AP, self) · Gillionaire (double gil, **stacks across all three girls up to ×8**) · Double Items (same stacking) · Covenant of Growth Grid (Double AP on the Blue gate, Double EXP on the Yellow) · AP Egg accessory (**Triple AP**) · Key to Success accessory (**Double All**).
...
1. **No published per-ability damage constants for player abilities.** SinirothX's Enemy Encyclopedia section 20 ("Complete Ability Listing") was never written; it is a stub in every released version. Enemy attack constants *are* published per enemy. The commonly quoted player-side values (Attack 16, Lv.1 magic 12, Lv.2 24, Lv.3 42, Flare 60, Ultima 70, Holy 100) come from a single third-party transcription and should be treated as a balance starting point.
2. **No ATB tick formula.** The relationship between Agility and gauge fill rate is qualitative only. The model in §1.2 is an estimate.
3. **No per-ability CTIM/RECTIM values in seconds.** Only the multipliers (−30/−40/−50/−80%, ×2 under Slow) are published, plus Trigger Happy's exact 1.8 / 2.2 / 2.6 s windows.
4. **No status durations in seconds.**
5. **No hit/evade formula.**
6. **ACCU/EVA/LUCK Up-Down magnitude is disputed** (±10 flat points vs ±1/12 multiplicative). See §2.8.
7. **Chain multiplier increment has one conflicting transcription** (0.5 vs 0.05). See §1.7.
8. Special dressphere stat derivation from node count is documented qualitatively ("more nodes = higher main-part stats") but **no formula is published**.
