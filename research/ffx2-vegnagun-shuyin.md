...
**CORRECTION — the Leg row's Reflect line (added 2026-09-21):** the Leg row's "Traps" cell above tells the player to **cast Reflect on the Leg**. §3.2 lists the Leg as immune "as Tail **plus Reflect**" under `[verified: 2 sources]`, the Node block immediately below it is "as Leg plus Def/MDef Up-Down" (so the Nodes are Reflect-immune too), and §3.5 restates the rule a third time: "unlike every Vegnagun part, which are Haste-immune (and all but the Tail Reflect-immune)", also `[verified: 2 sources]`. **The §7.2 table carries no confidence tag on any of its rows.** Per this document's own source-ranking policy (§0 and the §3 header, SinirothX treated as authoritative where sources conflict), **the immunity wins: Reflect cannot be cast on the Leg or on the Nodes.** This is one tagged statement against one untagged one, not two sources in conflict.

The row's second sentence — "Cast Reflect on **your** party too, and the Yellow Node's spells bounce back **at the Leg**" — is separately unsupported. `ffx2-combat-core.md`'s status table gives Reflect as *"bounces most **single-target** Black/White magic **back at the caster**; multi-target spells (e.g. Demi) are **not** reflected"*, `[verified: 2 sources]`. Two consequences follow from that row alone: a bounce returns to the **caster**, which for these spells is the **Node**, not the Leg; and of the Yellow set, §3.2's own ability table makes Firaga / Blizzaga / Thundaga / Waterga target **all chars**, leaving **Flare** (1 char) as the only one the verified rule would bounce at all.

**Disposition:** the Leg row's opener ("Buff to Protect + Haste on everyone") and core loop ("all damage goes to the Leg") stand; its Reflect line is **unsourced and superseded by this note**. The table cell is left as written, following the §3.5 Hero Drink precedent, where the conflict is recorded beside the claim rather than by editing it. Shipped code follows this note: `src/data/ffx2/enemies/vegnagun-leg.ts` carries `reflect: 255` for the Leg and all three Nodes, and the chapter-5 guide entry in `src/data/guides/ffx2-vegnagun-shuyin.ts` was corrected on 2026-09-21. **FFX-2 only** — this note says nothing about Reflect in the FFX chapters, which have their own dossiers.
...
| 17 | **Redoubt steal quantities.** §3.4's Redoubt row reads `Steal (50%) \| Phoenix Down / rare **Mega Phoenix**` and prints **no quantity for either item**, unlike every other steal row in §3 (Tail "X-Potion ×4 / rare ×6", Leg "Elixir ×1 / rare ×2", Node "Megalixir ×1 / rare ×2"). | **Open, unsourced.** `EnemyRewards.steal` needs a `count` per entry, and inventing one would break hard rule 6. A re-fetch of the FF Wiki *Redoubt* page on **2026-09-21** returned **HTTP 402**, and no jegged.com Redoubt page exists (§13.1 already records that jegged catalogues no Vegnagun sub-part). The shipped Redoubt records therefore carry **no steal table** until the quantity is sourced or Bailey rules on a default. |
| 18 | **The Node's rare drop.** §3.2 gives the Nodes `Drop (100%) \| Megalixir ×1 / **rare: Hero Drink ×1**` but publishes **no rate for the rare slot**. | **Open, unsourced.** `EnemyRewards.drops` is a flat `ItemDrop[]` with an optional per-entry `chance` percentage — it has no common/rare slot pair — so encoding the rare Hero Drink at all means inventing a rate (hard rule 6). It is therefore left out of the shipped Node record until a rate is sourced. Note this is the **drop** slot: the Nodes' *steal* table (Megalixir ×1 / rare ×2, 50%) is sourced and is shipped. |
...
| 26 | "Cast Reflect on the Leg", and Reflect on the party bouncing the Yellow Nodes' spells "at the Leg" (§7.2, Leg row) — checked 2026-09-21 | **Contradicted (internal)** | Contradicted by this document itself, not by an outside source. §3.2 (Leg "as Tail **plus Reflect**") and §3.5 ("all but the Tail Reflect-immune") both carry `[verified: 2 sources]`; the §7.2 table carries no tag on any row, so the doc's own source-ranking policy (§0, §3 header) settles it in favour of the immunity. The bounce half is contradicted by `ffx2-combat-core.md`'s Reflect row (`[verified: 2 sources]`), which returns a bounce **to the caster** and excludes multi-target spells — §3.2 makes four of the five Yellow abilities all-chars. Corrected in place: CORRECTION note added under the §7.2 table recording both halves; the table cell is left as written, per the row 20 precedent. Downstream: `src/data/guides/ffx2-vegnagun-shuyin.ts`'s Leg note rewritten the same day and pinned by `tests/unit/ffx2-vegnagun-reflect-immunity.test.ts` and `tests/unit/guide-status-immunity.test.ts`. |
...
### 13.2 Research sweep addendum (2026-09-23)

Sources read this pass: SinirothX, *Enemy Encyclopedia* v1.3 (GameFAQs faqs/31807, primary text, entries "Right Redoubt", "Left Redoubt", "Right Bulwark", "Left Bulwark", "Node A/B/C"); FF Wiki raw wikitext via the MediaWiki API: *Redoubt* (rev 3990466), *Bulwark (Final Fantasy X-2)* (rev 3990465), *Node (Final Fantasy X-2)* (rev 3999002), *Vegnagun (head)* (rev 4015591).

| # | Fact | Value | Confidence |
|---|---|---|---|
| S1 | **Redoubt steal (both Right and Left)** | common **Phoenix Down ×1**, rare **Mega Phoenix ×1**, steal rate **128** (/255, 50.2 %) | `[verified: 2 sources]` — FF Wiki *Redoubt* infobox prints `x1` for both slots and `steal rate = 128`; SinirothX prints `Phoenix Down/Mega Phoenix (50%)`. SinirothX writes a quantity only when it is above 1 (Leg `Elixir/Elixir x2`, Tail `X-Potion x4/X-Potion x6`), and the FF Wiki *Bulwark* infobox prints `Phoenix Down x1 / L-Bomb x1` for the row SinirothX writes `Phoenix Down/L-Bomb (50%)`, which confirms the convention. **Closes §11/§13 row 17.** |
| S2 | Stolen gil (the Pilfer Gil yield) | Redoubt **350** each, Bulwark **300** each (Oversoul 0) | `[verified: 2 sources]` — SinirothX `Stolen Gil`, FF Wiki `steal gil`. SinirothX's legend defines the field as "Gil you can steal from monster". |
| S3 | **The game names its duplicate parts itself** | **Node A, Node B, Node C** (bestiary #246, #247, #248), **Right Bulwark / Left Bulwark** (#250, #251), **Right Redoubt / Left Redoubt** (#253, #254) | `[verified: 2 sources]` — SinirothX `Monster's Name/Class` lines and the FF Wiki infobox section names + bestiary numbers; the *Vegnagun (head)* page's AI text also says "Right Redoubt, and Left Redoubt". Each is its own enemy record, so no Vegnagun formation contains two enemies with the same name. The parenthesised "(Tail)", "(Leg)", "(Core/Torso)", "(Head)" after "Vegnagun" are SinirothX's own disambiguation, not proven in-game text `[single source]`. |
