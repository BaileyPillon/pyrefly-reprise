// HP_KEY=<omnis|isaaru>. Show one hero-plate option on the real pause CHAPTER tab and the party-prep chapter card
// at 1600x900. Chapter I (FFX) hosts it: its plate URLs are answered with the option's
// bytes by Playwright request interception (public/art and src/ untouched). The chapter
// is not registered yet, so the host chapter's words are swapped for this chapter's in the DOM
...
const KEY = process.env.HP_KEY || 'omnis';
const SRC = `D:/Tools/pyrefly-scratch/hero-plates/${KEY}/options/`;
const OUT = `D:/Tools/pyrefly-scratch/hero-plates/${KEY}/shots/`;
...
async function open() {
  ...
  if (OPT !== 'orig') {
    ...
    const focal = JSON.parse(readFileSync(`D:/Final Fantasy/docs/concepts/chapters/${KEY}/hero-plate/scripts/focal.json`, 'utf8'))[OPT];
    await page.route((u) => new URL(u).pathname.endsWith(`/art/pause/${HOST}.json`), (r) =>
      r.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ subject: `${KEY} hero option ${OPT}`, focal }) }));
  }
...
const WORDS = JSON.parse(readFileSync(`D:/Final Fantasy/docs/concepts/chapters/${KEY}/hero-plate/scripts/words.json`, 'utf8'));
