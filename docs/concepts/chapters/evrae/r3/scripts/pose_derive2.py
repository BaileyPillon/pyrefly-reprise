"""hurt and cast from idle-near (FFX only), r3 repair cycle: pose_derive.py with the jaw rebuilt. The head
and neck rotate and shift as one layer over the fixed body; the lower jaw rotates about the mouth corner with
a weight that ramps across the mouth gap itself (width 0.26 r), so the opened mouth stretches the gullet's own
dark pixels and never folds the teeth. usage: SHIFTX=-55 SHIFTY=28 python pose_derive2.py <out.png> -11 520 230 350 600 -20"""
...
if jaw:
    # lower jaw (idle-near, measured at 4x on a 10 px grid): the mouth corner (hinge) is at
    # (305, 192); the gap between the upper and lower teeth runs from it to (248, 222). The weight
    # ramps across the gap itself, its width growing with the distance from the hinge (0.32 r),
    # so the open mouth stretches the gullet's own dark pixels and never folds the teeth.
    hx, hy = float(os.environ.get('HX', 305)), float(os.environ.get('HY', 192)) + PAD
    gx, gy = 248.0, 222.0 + PAD
    nx, ny = -(gy - hy), (gx - hx); nn = np.hypot(nx, ny); nx, ny = nx / nn, ny / nn   # normal to the gap line
    if ny < 0: nx, ny = -nx, -ny                                                       # pointing down, into the lower jaw
    dist = (xs - hx) * nx + (ys - hy) * ny
    r = np.hypot(xs - hx, ys - hy)
    wid = np.maximum(3.0, float(os.environ.get("JW", 0.26)) * r)
    wj = smooth(dist / wid + 0.5) * smooth((hx + 4 - xs) / 22) * smooth((275 + PAD - ys) / 12) * smooth((xs - float(os.environ.get("JX0", 232))) / float(os.environ.get("JXW", 14)))
