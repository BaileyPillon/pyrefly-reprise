#!/usr/bin/env node
/**
 * Chapter 6 (FFX-2 only): train the logos-x2 identity LoRA, ROUND 2 (copy of ../train.mjs with
 * the round-2 dataset, out-r2 and the name logos-x2-r2; settings unchanged).
 *
...
const NAME = 'logos-x2-r2';
const ROOT = join(LORA_ROOT, 'logos');
const DATA = join(ROOT, 'dataset-r2');
const OUT = join(ROOT, 'out-r2');
...
function takeLock() {
  try {
    ...
    writeSync(fd, JSON.stringify({ owner: 'logos-x2-r2 (pyrefly-leblanc-lora-r2)', pid: process.pid, at: stamp() }));
    closeSync(fd);
    return true;
  } catch {
    return false;
...
async function train(a) {
  ...
  const toml = join(ROOT, 'dataset-r2.toml');
...
if (a._[0] === 'gate') await gpuGate();
  ...
  if (!existsSync(join(DATA, 'manifest.json'))) throw new Error('build the dataset first: round2/build-dataset-r2.py build');
  await train(a);
