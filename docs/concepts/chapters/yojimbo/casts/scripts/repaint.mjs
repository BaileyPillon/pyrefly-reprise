#!/usr/bin/env node
/**
 * Yojimbo casts (FFX only): masked repaint of one prepared crop. Animagine XL 4.0 Opt, no LoRA,
...
const LEDGER = 'D:/Tools/pyrefly-scratch/yoj-casts/gpu-ledger.json';
const CAP_MIN = 60;
...
function graph({ init, mask, refs, seed, denoise, words, negative, refWeight }) {
  return {
    ...
    9: { class_type: 'SaveImage', inputs: { filename_prefix: 'pyrefly/yoj-casts', images: ['8', 0] } },
  };
}
...
async function run(workflow, outPath, note) {
  ...
  const res = await fetch(`${COMFY}/prompt`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ prompt: workflow, client_id: 'pyrefly-yoj-casts' }) });
