"""Cut a crop of a collage + mask, upscale to a 1024 short side on white, write <job>.init/.mask.png.
   ...
   python prep.py refs <subject-key> <x0> <y0> <x1> <y1> <outdir>   (idle square-padded + detail crop)
"""
import sys, json
...
if sys.argv[1] == 'refs':
    key = sys.argv[2]; x0, y0, x1, y1 = map(int, sys.argv[3:7]); out = sys.argv[7]
    idle = load(IDLE[key]); H, W = idle.shape[:2]; s = max(H, W)
