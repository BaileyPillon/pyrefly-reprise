#!/usr/bin/env node
/**
 * Yojimbo casts (FFX only): in-battle captures at 1600x900 on the real Chapter IX battle.
 *
 *   PYREFLY_BROWSER=gpu node docs/concepts/chapters/yojimbo/casts/scripts/ingame.mjs
 *
 ...
 * cast held, Daigoro's cast held, and Yojimbo's flinch caught mid-way (actor.flinch(340), 6 frames in).
 */
...
const OUT = 'D:/Tools/pyrefly-scratch/yoj-casts/ingame';
...
const VARIANTS = {
  none: { yojimbo: { idle: IDLE_YOJ } },
  A: { yojimbo: { idle: IDLE_YOJ, cast: 'yojimbo-cavern/cast.png', hurt: 'yojimbo-cavern/hurt.png' }, daigoro: { cast: 'daigoro/cast.png' } },
  B: { yojimbo: { idle: IDLE_YOJ, cast: 'yojimbo-cavern/cast.png' }, daigoro: { cast: 'daigoro/cast-alt-narrow.png' } },
};
...
try {
  for (const [v, swap] of Object.entries(VARIANTS)) {
    ...
    const mid = await page.evaluate(async () => {
      const p = window.__pyrefly; const stage = p.battle().stage; const a = stage.actor('yojimbo');
      for (const other of stage.staged()) stage.actor(other).setPose('idle', { immediate: true, force: true });
      await p.shot('enemy', 90);
      a.flinch(340);
      await p.shot(undefined, 6);
      const slot = a.slots[a.active];
      return { id: 'yojimbo', pose: a.pose, rect: stage.projectRect('yojimbo'), meta: { width: slot.meta.width, height: slot.meta.height } };
    });
    await snap('yojimbo-flinch', mid);
