...
const [plate, boss, out, hudArg] = process.argv.slice(2); // production: the installed plate and the staged composite
const pngSize = (p) => { const b = readFileSync(p); return { w: b.readUInt32BE(16), h: b.readUInt32BE(20) }; };
const side = (p) => { const s = pngSize(p); return JSON.stringify({ width: s.w, height: s.h, baselineY: s.h - 2, pose: 'idle', composition: 'boss', nonBiped: true, facing: 'left', status: 'CANDIDATE' }); };
...
await page.route('**/art/portraits/braskas-final-aeon*.png*', r => r.fulfill(png(process.env.BOSS_ICON || 'D:/Final Fantasy/public/art/portraits/seymour-omnis.png')));
