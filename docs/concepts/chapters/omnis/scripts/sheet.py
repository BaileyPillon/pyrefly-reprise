...
PAD = 40; GAP = 30; LABEL_H = 84; CAP_H = 112
...
H = PAD + 150 + sum(LABEL_H + ch + CAP_H + GAP for _, _, _, ch in rows) + PAD
sheet = Image.new('RGB', (W, H), INK); d = ImageDraw.Draw(sheet)
d.rectangle((0, 0, 10, H), fill=GOLD)
d.text((PAD, PAD), spec['title'], font=font(64, True), fill=PAPER)
d.text((PAD, PAD + 80), spec.get('subtitle', ''), font=font(32), fill=DIM)
y = PAD + 150
for row, ims, cw, ch in rows:
    d.text((PAD, y + 12), row['label'], font=font(48, True), fill=GOLD)
    y += LABEL_H
    for k, (it, im) in enumerate(zip(row['items'], ims)):
        ...
        if it.get('key'):
            d.rectangle((x, y, x + 84, y + 72), fill=GOLD)
            d.text((x + 22, y + 6), it['key'], font=font(56, True), fill=INK)
        cap = it['cap']; words = cap.split(' '); lines = ['']
        for w_ in words:
            t_ = (lines[-1] + ' ' + w_).strip()
            if d.textlength(t_, font=font(38)) > cw and lines[-1]:
                lines.append(w_)
            else:
                lines[-1] = t_
        for li, ln in enumerate(lines[:2]):
            d.text((x, y + ch + 8 + li * 46), ln, font=font(38), fill=PAPER)
    y += ch + CAP_H + GAP
d.text((W - 260, H - 56), 'CONCEPT', font=font(40, True), fill=GOLD)
