...
const LEDGER = 'D:/Tools/pyrefly-scratch/gpu2/omnisportrait/gpu-ledger.json';
const CAP_MIN = 40;
...
function graph({ mode, init, mask, refs, seed, denoise, words, negative, refWeight, w, h }) {
  const g = {
    ...
    9: { class_type: 'SaveImage', inputs: { filename_prefix: 'pyrefly/omnis-portrait-opt', images: ['8', 0] } },
  };
  let latent;
  if (mode === 'img2img') {
...
async function run(workflow, outPath, note) {
  ...
  const res = await fetch(`${COMFY}/prompt`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ prompt: workflow, client_id: 'pyrefly-omnis-portrait-opt' }) });
...
const init = mode !== 'txt2img' ? stageImage(`${job}.init.png`) : null;
...
for (let i = 0; i < seeds; i++) {
  ...
  writeFileSync(out.replace(/\.png$/, '.json'), JSON.stringify({
    mode, init: init && `${job}.init.png`, tag, mask: mask && `${job}.mask.png`, seed, denoise: mode !== 'txt2img' ? d : 1, words: a.words, negative, model: CKPT,
