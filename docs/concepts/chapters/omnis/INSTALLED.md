...
| `portraits/seymour-omnis.png` + `.json` | B17 c | 832x1216 | `55968b5d9a49` | A new render (seed 925213, round 2 of 2, pick 3 of 4) drawn from the O-1 A idle's head-and-shoulders crop through IP-Adapter at 0.3. It uses a white-ground matte; the RGB equals the raw render exactly inside the alpha. Round 1 (ref 0.4 on the head plus the idle) came out neon and invented head horns and forehead marks. Pupils (read off a 2x gridded crop): (270, 437) and (439, 357), so fx 0.4261, fy 0.3265, ipd 0.2247. That is ready for a `src/ui/common/face-crops.json` row once a speaker id uses it. The row is not added because this track commits docs only. |
...
- The idle's outermost strips are cut by the render's own canvas edge: 355 px of straight edge on the
  ...
  (103.8 % x 101.1 %; one component, near-white fine), for the same reason the options run quarantined
...
- The portrait has red and orange trim and cone spikes on the shoulders that the idle does not have,
  and no red eye on the shoulder. A white forehead highlight may bloom under the dialogue grade. No
  options round was shown for the portrait itself: O-5 was to be drawn from the O-1 pick, so this is
  the first time Bailey sees it.
