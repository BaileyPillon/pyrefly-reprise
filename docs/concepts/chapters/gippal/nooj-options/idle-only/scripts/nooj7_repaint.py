"""Nooj shade attempt 7 (idle-only repair; copied from nooj6_repaint.py, paths changed) (FFX-2 only): crop-upscale masked repaint with the IP-Adapter forced on the padded portrait.
...
tmp = 'D:/Tools/pyrefly-scratch/nooj7/tmp'
...
g = upload(gp, f'nooj7-g-{tag}.png'); m = upload(mp, f'nooj7-m-{tag}.png')
rnames = [upload(flat_png(r), f'nooj7-r{i}-{tag}.png') for i, r in enumerate(refs)]
...
wf = {
    ...
    '10': {'class_type': 'SaveImage', 'inputs': {'images': ['9', 0], 'filename_prefix': 'pyrefly/nooj7/repaint'}},
}
...
req = urllib.request.Request(BASE + '/prompt', data=json.dumps({'prompt': wf, 'client_id': 'pyrefly-nooj7'}).encode(), headers={'Content-Type': 'application/json'})
...
if mx < 8:
    open('D:/Tools/pyrefly-art-backup/candidates/2026-09-25-nooj7/STOP-BLACK.txt', 'a').write(f'{time.ctime()} {a.out} all-black\n')
...
res.save(a.out)
json.dump({'game': 'ffx2', 'subject': 'nooj-shade', 'status': 'CANDIDATE', 'attempt': 7, 'kind': 'masked repaint',
