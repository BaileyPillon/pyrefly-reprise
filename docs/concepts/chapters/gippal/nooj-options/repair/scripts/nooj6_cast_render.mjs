#!/usr/bin/env node
/**
 * Nooj shade attempt 6 (FFX-2 only, Den of Woe): option 1's cast re-derived from the REPAIRED idle's identity.
 * A copy of attempt 5's nooj5_render.mjs (same graph: IP-Adapter forced on the padded portrait + head crop + the
 * repaired idle, xinsir OpenPose on attempt 5's own cast skeleton, core regional masks, arm C img2img) with only
 * these changes: the block-in, masks and refs come from nooj6_cast_prep.py; the fur region is the painted fur's own
 * shape on the far shoulder and its words drop 'fur collar' and 'purple sleeve'; the arm region names the idle's
 * dark blue arm and grey hand; the negative adds collar and both-shoulder fur. Candidates only; nothing is installed.
 *
...
const OUT = 'D:/Tools/pyrefly-art-backup/candidates/2026-09-25-nooj6';
const PREP = 'D:/Tools/pyrefly-scratch/nooj6/prep';
const PREP5 = 'D:/Tools/pyrefly-scratch/nooj5/prep';
...
const IDLE_REF = flag('idleRef', `${PREP}/ref-idle-repaired-square.png`);
...
const IDENTITY = 'nooj \\(ff10-2\\), final fantasy x-2, male focus, (mature male:1.2), adult man, tall, broad shoulders, masculine angular face, sharp jawline, brown hair, hair rings, ' +
  ...
  'red and black belts, multiple belts, fur shoulder, grey fur, (mechanical arm:1.2), prosthetic arm, black glove, human ears, five fingers, silver cane, purple boots';
const POSE_TAGS = {
...
const REGIONS = {
  fur: '1boy, (grey fur:1.2), fur-trimmed shoulder, fluffy fur, dark red sleeve',
  arm: '1boy, (mechanical arm:1.3), robot arm, prosthetic arm, thin dark blue metal arm, metal shoulder, exposed joints, grey metal hand, jointed fingers, open hand',
  shin: '1boy, mechanical leg, black metal shin, piston, exposed joints, metal foot',
};
const NEG = `${SPRITE_NEGATIVE}, multiple views, 2boys, 1girl, chibi, sketch, monochrome, 3d, realistic, cropped, from behind, facing away, ` +
  ...
  'fur collar, fur around neck, white fur, fur on both shoulders, shoulder armor, shoulder pad, fan, folding fan, white arm, ' +
  'dual wielding, holding two weapons, trident, fork, polearm, staff behind back, weapon on back, armored sleeve';
...
async function main() {
  ...
  const staged = {
    skel: stageImage(`${PREP}/cast-skel.png`),
    blockin: stageImage(`${PREP}/cast-blockin.png`),
    masks: Object.fromEntries(['fur', 'arm', 'shin'].map((r) => [r, stageImage(`${PREP}/cast-mask-${r}.png`)])),
  };
  const refFiles = [`${PREP5}/ref-portrait-square.png`, `${PREP5}/ref-portrait-head.png`, ...(IDLE_REF ? [IDLE_REF] : [])];
  ...
  while ((jobs.length || inflight.length) && !stop) {
    while (jobs.length && inflight.length < 3 && (await pending()) < 3) {
      const j = jobs.shift();
      const { g, denoise } = graph({ pose, arm, seed: j.seed, refs, staged, prefix: `pyrefly/nooj6/${pose}-${arm}` });
      const res = await fetch(`${COMFY}/prompt`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ prompt: g, client_id: 'pyrefly-nooj6' }) });
      ...
      console.log(`[nooj6] queued ${pose}/${arm} seed ${j.seed}`);
    }
    await sleep(2500);
    for (const j of [...inflight]) {
      const e = (await (await fetch(`${COMFY}/history/${j.id}`)).json())[j.id];
      if (!e) continue;
      if (e.status?.status_str === 'error') { console.error(`[nooj6] FAILED ${JSON.stringify(e.status.messages).slice(0, 1500)}`); inflight.splice(inflight.indexOf(j), 1); continue; }
      ...
      const side = {
        ...
        controlnet: { file: CONTROLNET, strength: CN, start: 0, end: 1, skeleton: 'nooj5_prep.py poses().cast, drawn by nooj6_cast_prep.py as cast-skel.png' },
        ipadapter: { file: IPADAPTER, ...IPA, combine: 'concat', images: refFiles.map((f) => f.split('/').pop()), forced: true },
        regions: arm === 'A' ? null : Object.fromEntries(Object.entries(REGIONS).map(([r, t]) => [r, { text: t, mask: `cast-mask-${r}.png`, strength: 1.0 }])),
        attempt: 6, derivedFrom: 'idle-repair/idle-repaired.png (option 1 idle 975102, repaired)',
        img2img: arm === 'C' ? { image: 'cast-blockin.png (nooj6_cast_prep.py)', denoise: j.denoise } : null,
      ...
      console.log(`[nooj6] ${pose}/${arm} seed ${j.seed}: ${cutMeta ? `${cutMeta.width}x${cutMeta.height}` : '-'} guard ${guard.ok ? 'ok' : 'REJECT ' + guard.reasons.join('; ')}${black ? ' BLACK' : ''} ${Math.round(side.seconds)}s`);
    }
  }
  if (stop) { console.error('[nooj6] BLACK FRAME: stopped'); process.exit(3); }
}
await main();
