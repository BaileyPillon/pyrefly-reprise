...
await page.goto('http://127.0.0.1:5820/', { waitUntil: 'commit', timeout: 150000 });
...
if (names.length) await page.evaluate((names) => {
  const fix = () => { const w = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
    let n; while ((n = w.nextNode())) for (const [a, b] of names) { if (a.startsWith('^')) { if (n.nodeValue.trim() === a.slice(1)) n.nodeValue = n.nodeValue.replace(a.slice(1), b); } else if (n.nodeValue.includes(a)) n.nodeValue = n.nodeValue.split(a).join(b); } };
  fix(); window.__fixNames = fix;
