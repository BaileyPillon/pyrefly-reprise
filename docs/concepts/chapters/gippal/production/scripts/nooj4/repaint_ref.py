...
tmpdir = 'D:/Tools/pyrefly-scratch/gpu3/den-omnis/tmp'
...
g = upload(gp, f'gpu3-den-g-{tag}.png'); m = upload(mp, f'gpu3-den-m-{tag}.png')
rn = upload(ref, f'gpu3-den-r-{tag}.png') if ref else None
wf = {
 ...
 '10': {'class_type': 'SaveImage', 'inputs': {'images': ['9', 0], 'filename_prefix': 'gpu3-den/repaint'}},
}
if rn:
...
req = urllib.request.Request(BASE + '/prompt', data=json.dumps({'prompt': wf, 'client_id': 'gpu3-den'}).encode(), headers={'Content-Type': 'application/json'})
