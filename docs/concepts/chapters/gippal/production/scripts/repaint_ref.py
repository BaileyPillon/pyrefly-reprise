...
args = sys.argv[9:]
alpha_mode = args[args.index('--alpha') + 1] if '--alpha' in args else 'keep'
ref = args[args.index('--ref') + 1] if '--ref' in args else None
ref_w = float(args[args.index('--ref-weight') + 1]) if '--ref-weight' in args else 0.55
...
tmpdir = 'D:/Tools/pyrefly-scratch/ch1215/gippal-repair/tmp'
...
g = upload(gp, f'gip-rep-g-{tag}.png'); m = upload(mp, f'gip-rep-m-{tag}.png')
rn = upload(ref, f'gip-rep-r-{tag}.png') if ref else None
wf = {
 ...
 '10': {'class_type': 'SaveImage', 'inputs': {'images': ['9', 0], 'filename_prefix': 'gip-repair/repaint'}},
}
if rn:
    wf['20'] = {'class_type': 'LoadImage', 'inputs': {'image': rn}}
    wf['21'] = {'class_type': 'IPAdapterModelLoader', 'inputs': {'ipadapter_file': 'ip-adapter-plus_sdxl_vit-h.safetensors'}}
    wf['22'] = {'class_type': 'CLIPVisionLoader', 'inputs': {'clip_name': 'CLIP-ViT-H-14-laion2B-s32B-b79K.safetensors'}}
    wf['23'] = {'class_type': 'IPAdapterAdvanced', 'inputs': {'model': ['1', 0], 'ipadapter': ['21', 0], 'image': ['20', 0], 'weight': ref_w,
                'weight_type': 'linear', 'combine_embeds': 'concat', 'start_at': 0.0, 'end_at': 1.0, 'embeds_scaling': 'K+V', 'clip_vision': ['22', 0]}}
    wf['8']['inputs']['model'] = ['23', 0]
req = urllib.request.Request(BASE + '/prompt', data=json.dumps({'prompt': wf, 'client_id': 'gip-repair'}).encode(), headers={'Content-Type': 'application/json'})
...
json.dump({'seed': int(seed), 'denoise': float(denoise), 'prompt': pos, 'negative': neg, 'model': 'animagine-xl-4.0-opt.safetensors',
           ...
           'execSeconds': exec_s, 'ref': ref, 'refWeight': ref_w if ref else None}, open(out[:-4] + '.json', 'w'), indent=1)
