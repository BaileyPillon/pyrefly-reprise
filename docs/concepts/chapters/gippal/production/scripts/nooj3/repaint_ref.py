...
tmpdir = 'D:/Tools/pyrefly-scratch/gpu2/den/tmp'
...
g = upload(gp, f'gpu2-den-g-{tag}.png'); m = upload(mp, f'gpu2-den-m-{tag}.png')
rn = upload(ref, f'gpu2-den-r-{tag}.png') if ref else None
wf = {
 ...
 '10': {'class_type': 'SaveImage', 'inputs': {'images': ['9', 0], 'filename_prefix': 'gpu2-den/repaint'}},
}
if rn:
...
req = urllib.request.Request(BASE + '/prompt', data=json.dumps({'prompt': wf, 'client_id': 'gpu2-den'}).encode(), headers={'Content-Type': 'application/json'})
