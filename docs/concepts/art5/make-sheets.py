"""Per-dressphere review sheets for the art5 party pose candidates (FFX-2 only; a copy of the
Chapter XIII make-sheets.py with the art5 paths and no arm folders).
...
D:/Tools/pyrefly-art-backup/candidates/2026-09-25-gpu5/<girl-dressphere>/<pose>/.
"""
...
CAND = pathlib.Path('D:/Tools/pyrefly-art-backup/candidates/2026-09-25-gpu5')
...
def main():
    ...
    for girl, slots in verdicts['girls'].items():
        ...
        for pose in POSES:
            ...
            d = CAND / girl / pose
            v.setdefault('look', {})
            v['cands'] = v.get('cands') or sorted(int(q.stem.split('-')[1]) for q in d.glob('cand-*.json'))
        ...
        for pose, v, _ in rows:
            if v.get('pick'):
                strip.append((pose, flat(CAND / girl / pose / f"cand-{v['pick']}.png", gs)))
        strip_w = sum(t[1].width + 20 for t in strip) + 40
        W = max(W, strip_w)
        H = 70 + sum(row_h) + GAME_H + 120
        sheet = Image.new('RGB', (W, H), (250, 250, 250))
        dr = ImageDraw.Draw(sheet)
        dr.text((20, 18), f"{girl}: battle-pose CANDIDATES, art5 (FFX-2 only). Nothing installed. Looks are an agent's, not a judge's and not Bailey's. "
