#!/usr/bin/env node
/**
 * Living portrait v5.1 (game case: both; the plate is Yuna X-2): the LoRA repaint of one propagated key's
 ...
 *
 *   node docs/concepts/pause-until-dawn/prototype-v2/shots/v51/tools/chain-repaint.mjs
 *        --init <init.png> --mask <mask-rep.png> --yaw 20 --seed 5500 --out <dir>/cand
 */
...
async function main() {
  ...
  const g = graph({ yaw, seed, lora: 'yuna-x2.safetensors', strength: 0.8, cn: 0, cnEnd: 1, refWeight: 0.3, pose: null,
    ref: stageImage('public/art/portraits/yuna-x2.png'), prefix: 'pyrefly/v51', init: stageImage(init), mask: stageImage(mask), denoise: denoise[0], negative });
  ...
  denoise.forEach((d, i) => {
    ...
    g[n + 2] = { class_type: 'SaveImage', inputs: { filename_prefix: `pyrefly/v51-d${Math.round(d * 100)}`, images: [String(n + 1), 0] } };
  });
  console.log('[v5] waiting for 3 quiet minutes on the shared queue');
  await waitQuiet();
  const t0 = Date.now();
  const res = await fetch(`${COMFY}/prompt`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ prompt: g, client_id: 'pyrefly-v51' }) });
