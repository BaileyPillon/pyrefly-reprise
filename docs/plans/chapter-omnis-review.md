...
| Mortiphasm `m106` ×4 | 1 HP, **immune to all damage**, every status, Scan and Sensor; out of melee reach (only Wakka, Valefor, Anima, Mindy reach physically); **no turns of its own** `[estimate]` (the decompile lists no actions, the wiki lists Turns Left / Right; B22) | §2 `[decompiled]`; reach `[verified: 4]`; turns `[estimate]` (review E6) |
...
| **Unsourced** | **O-7 the colour order around a disc** and **O-11 the reset cycle** (block the lock); O-4 spell-to-member mapping; O-8 which spells turn a disc; O-9 whether disc hits count; O-10 reset timing (2 sources say next turn, 1 says straight after; B23); disc turns in the queue (B22); O-12 DEF writes vs Armor Break; O-2 Threaten; O-5 Thunder ↔ Water | §10 |
...
| **B8** | **The two blocking unknowns**: the colour order around a disc (O-7) and the reset cycle (O-11) | a) Bailey confirms both from his own memory or a video he watches / b) build GameFAQs' cycle Fire → Water → Ice → Thunder and a ring order labelled "our estimate", and list the chapter only once confirmed | **b, then a** before the chapter is listed; no download (rule 11). The option sheets draw GameFAQs' *reset cycle* as the *physical order around a disc*: two guesses stacked, both labelled; a confirmation must cover both (review, settled-list 3) |
...
| **B12** | Which disc's spell hits whom, and which disc's spell is dropped when a member is KO'd | a) discs left to right onto living members in slot order, the fourth random; with a member KO'd, the first discs keep their spells / b) all random | **a**, labelled estimate (both halves unsourced; review S4) |
...
| **B22** | Do the discs take turns of their own? (added by the review, E6) | a) no: they turn only when hit, and the queue shows no disc tiles / b) each disc owns a CTB slot and four disc tiles sit in the queue | **a**, `[estimate]`: the decompile lists no actions for `m106`, and no source gives a disc a turn. The option frames already show a (the discs were taken out of the queue) |
| **B23** | When the discs reset after Ultima (O-10; added by the review, S8) | a) on his next turn after Ultima (wiki *Mortiphasm*, GamerGuides: 2 sources) / b) straight after Ultima (GameFAQs: 1 source) | **a**, by the majority rule; one constant either way |
...
| T3 AI script O-G5, O-G6 | `src/battle/ffx/ai/seymour-omnis{,-rules}.ts`, registration | T1, T2, B8–B12, B22, B23 | 1.5 | opus |
...
**Order.** NOW: Bailey answers B1–B23 on one sheet; O-2 first, then O-1, O-3…O-6; T1, T2, T4 and
...
- **Mechanic units, one per research row:** opening four Firaga, weak to Ice; the -ra / -ga
  ...
  Ultima ignores Shell, respects Focus and Shield, not reflectable; reset to the next element on the timing B23 picks;
...
the volley, the affinity event, the disc stage. Nothing perceivable before B1–B23 and O-1…O-6.
...
**Corrections applied (2026-09-24, after this review):** S8 → B23 and research O-10; E6 → §2 row and B22; S4 → B12;
S11 typo fixed in research §3.3; A2 ("hurts him extra", ×1.5), A3 (O-3 subtitle), A4 (README) and A5 (question 1) fixed on the
sheets and README in `docs/concepts/chapters/omnis/`, whose sheets were re-cut one column for a phone.
