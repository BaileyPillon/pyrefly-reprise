...
14. Release-10 fix agents (plain agents, launched 09:50 EDT 09-23, commit to main without pushing; the driver validates and pushes): (a) Sonnet, PR-0122 + PR-0123 in `src/ui/common/EnemyIntent.ts`, `enemy-intent.css`, the pause hooks, `src/battle/ffx2/intent.ts` (BOTH games); (b) Sonnet, PR-0126 advisor "in <submenu>" chip in `src/ui/ffx2/advisorLane.ts` / `src/ui/common/MoveAdvisor.ts` + PR-0124 HP/MP clamp at chain seams in `BattleScreenSetup.ts` / `src/battle/ffx2/setup.ts` (FFX-2 only; the dressphere carry stays with Bailey, rule 6); (c) Opus, PR-0125 Doublecast spell + target steps and the engine safety net in `src/ui/ffx/CommandMenu*.ts`, `src/battle/ffx/execute.ts`, `src/data/ffx/**` (FFX only). If one of these is you: carry on.
