...
5. `wf_cc2af145-0bf` "pyrefly-living-portrait-v3" DONE 22:50 EDT (0236347, 3f0cf66, 6caf0ef art; c3dc63d mesh warp + turn reversal fixed; check refuted v3.1; v3.3 44d39a2 / c4eacfc / d93caa5: one painting at a time, backs of heads outpainted, per-key lids, rest = plate to the pixel, collar seam gone; `docs/handoff/living-portrait-v3.md`). Remaining: the turned keys are different paintings (brown hair, no tassel), so a turn reads as a change of picture. `wf_39f6741a-f84` "pyrefly-living-portrait-v4" (launched 22:55 EDT) fixes that: a Yuna identity LoRA (kohya, approved) + OpenPose keys every 20 degrees, SAM 2 masks, expressions on turned keys, adversarial check; OWNS prototype-v2/**, tools/gen/rig-*, tools/gen/lora-*, D:/Tools/pyrefly-lora/**. GPU rule: LoRA training waits for an empty ComfyUI queue and under 4 GB used. Was: silhouette masks, hidden-region inpainting, mesh warp, colour-matched
