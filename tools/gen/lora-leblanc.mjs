 ...
 * and the lock file D:/Tools/pyrefly-lora/gpu-train.lock (exclusive create,
 * protocol in D:/Tools/pyrefly-lora/GPU-LOCK.md) must be free. The lock is held
 * for the whole training and deleted at the end. ComfyUI is never restarted.
 * Renders are queued one at a time behind an empty queue.
 */

import { spawn, execFileSync } from 'node:child_process';
import { closeSync, createWriteStream, existsSync, mkdirSync, openSync, readFileSync, unlinkSync, writeFileSync, writeSync } from 'node:fs';
import { join, resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { SPRITE_NEGATIVE, STYLE_TAGS, QUALITY_TAGS, stageImage } from './comfy.mjs';
...
/** The installed idle padded square on white (pilot 2's ref; method check 2.2: the adapter centre-crops a square). */
const IDLE_SQUARE = join(REPO, 'docs/concepts/chapters/leblanc/pilot2/refs/idle-square.png');
const GATE_MB = 4096;
...
async function run(workflow, outPath) {
  ...
  while (!(await queueEmpty())) {
    if (Date.now() - noted > 60_000) { console.error('[leblanc] queue busy; waiting'); noted = Date.now(); }
    await sleep(5000);
  }
  const t0 = Date.now();
  const res = await fetch(`${COMFY}/prompt`, {
    ...
    body: JSON.stringify({ prompt: workflow, client_id: 'pyrefly-lora-leblanc' }),
  });
...
// Step test prompts: trigger + view + pose only; the LoRA carries the identity.
export const PROMPTS = {
  // the idle pose, worded as the dataset captions word it
  idle:
    'leblancX2, 1girl, solo, full body, standing, looking at viewer, smile, holding folding fan, closed fan, ' +
    'fan to mouth, arm up, holding clothes, feet, white background, simple background',
  // a pose the dataset never shows (does the identity hold off the idle's pose, or is it memorised?)
  attack:
...
export function testGraph({ lora, strength, ref, refWeight, seed, prompt }) {
  const g = {
    4: { class_type: 'CheckpointLoaderSimple', inputs: { ckpt_name: CKPT } },
    5: { class_type: 'EmptyLatentImage', inputs: { width: 832, height: 1216, batch_size: 1 } },
    8: { class_type: 'VAEDecode', inputs: { samples: ['3', 0], vae: ['4', 2] } },
    9: { class_type: 'SaveImage', inputs: { filename_prefix: 'pyrefly/lora-leblanc', images: ['8', 0] } },
  };
  let model = ['4', 0];
  let clip = ['4', 1];
  if (lora) {
  ...
  for (const l of loras) {
    ...
    for (const seed of seeds) {
      ...
      writeFileSync(`${out}.json`, JSON.stringify({
        ...opts, pose, negative: NEGATIVE, model: CKPT,
        ipadapter: { file: IPADAPTER, weight: refWeight, type: 'ease in', start: 0.2, end: 0.6, scaling: 'K+V', image: 'docs/concepts/chapters/leblanc/pilot2/refs/idle-square.png (the installed idle square-padded on white)' },
        steps: 28, cfg: 6, sampler: 'euler_ancestral', scheduler: 'normal', width: 832, height: 1216, seconds: s,
      }, null, 1));
      console.log(`[leblanc] ${out}.png ${s.toFixed(1)} s`);
    }
  }
}
...
if (isMain) {
  const a = args();
  const cmd = a._[0];
  if (cmd === 'gate') { await gpuGate(console.log); releaseLock(); }
  else if (cmd === 'train') {
  ...
  else console.log('usage: node tools/gen/lora-leblanc.mjs gate | train [--steps N --dim N --alpha N --lr X] | steptest [--loras a,b --seeds 1,2 --pose idle|attack]');
}
