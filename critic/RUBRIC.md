...
- **Approved artwork is protected.** Check its identity, crop, resolution, framing, staging and integration. Never replace it because a reviewer prefers something else (`docs/target/approved-hashes.json`, and the judge-locked sets in `docs/target/judge-locked-hashes.json`, enforced the same way). A separately approved face pass authorises only that named change.
...
Category scores run 0–10 with one decimal; total = Σ(score × weight) / 100, **tested unrounded against 9.60** (9.595 is not acceptance). Anchors: 5 = substantial gaps or placeholder delivery; 7 = functional with conspicuous weaknesses; 9 = polished with minor issues; 9.6 = the declared target delivered with only negligible defects; 10 = no material gap found in the complete tested scope. These are project acceptance judgments, not an objective certification. They are calibrated against `critic/calibration/cases.json`: cases with a known answer taken from Bailey's own verdicts and from the critic's recorded mistakes (see §9).
...
Each required target has an owner decision, a reference and version, a game and release scope, the selected properties, the matching build state and concrete acceptance cases. `docs/target/targets.json` records the **design status** (approved / awaiting decision / rejected / superseded); the **delivery status** (not scheduled / in progress / implemented / verified) sits beside it in each tile's optional `delivery` field (adopted 2026-09-21). Only a validated report moves a tile to `verified`, and the tile then names it in `verifiedBy` (report path and main sha); a tile without the field is unknown, never assumed built. Bailey's decisions that are not pictures live in `docs/target/decisions.json` with the states proposed / adopted / deferred / rejected / superseded / verified: an adoption is not a verification. Approved tiles are decision counts, not verified implementations; tiles that approve an audio direction or key art are not scored as screenshot matches.

**What Bailey named, and what an agent guessed (adopted 2026-09-21).** A tile or group may carry a `reaction`: Bailey's words and date, then `liked`, `disliked`, `mustRemain`, `mustChange`, `undecided`, and separately `inferred` for anything an agent assumed. Acceptance cases come only from `mustRemain`, `mustChange`, `disliked` and named `liked` items. An `inferred` or `undecided` item never fails a build: it appears in the report as a question for Bailey, and a builder asks before building it. Behaviour the sources establish as canon stays a requirement whatever the reaction says (AGENTS.md rules 6 and 14): a reaction records taste, not fidelity. Reactions are recorded going forward and where the words are already on record; nobody back-fills one from guesswork.
...
**The same limit holds for an area (adopted 2026-09-21).** When two consecutive reviews leave the same issue ID open at the same severity, or leave a category that the work targeted without a gain, the next batch in that area starts with a written method check instead of a third similar batch: the current route and why it stalled, up to two alternatives, the smallest test that would tell them apart, and one choice of continue / change method / small probe / defer / ask Bailey. A cosmetic gain does not reset the count. What counts as a gain for a category is stated by the reviewer in the report, and Bailey can overrule it. `npm run critic:status` prints `STALLED:` for every such issue (`stalledIssues`, `cadence.stalledAfterReviews`); it informs the plan and never blocks a deploy.

**Count the attempts, cap the cycles.** An issue in a report may carry `attempts`, the number of repairs already tried; `validateReport` checks it and the status tool flags an issue that reaches `repairAttemptsBeforeEscalation`. A release candidate gets two repair and recheck cycles in normal usage mode, one in conserve, none in protect (`cadence.repairCyclesPerCandidate`; modes in §9). Before another cycle: a usage reading, the method check above, and one of (a) take the failing change out of the candidate and ship what passed, (b) defer the candidate, (c) ask Bailey. Taking a change out is a new commit made in the release worktree `D:\pyrefly-release`; it is never a `reset`, a `restore` or a `stash` in the shared tree, and the plan is re-run on what remains.
...
**Usage modes (adopted 2026-09-21; `usageModes` in policy.json).** The mode follows the weekly allowance still left; the 5-hour window uses the same thresholds and can only tighten it.

| Mode | Weekly allowance left | What it permits |
|---|---|---|
| Normal | more than 50 percent | Workflows as planned, at most two at once. Opus for engine, presenter and verification, Sonnet for well-specified tracks. Two repair cycles per candidate. |
...
Pacing warning: when the allowance left trails the share of the week left by more than five points, drop one mode (71 percent left with 96 percent of the week left is conserve, not normal). **Bailey sets the mode by word; agents may only tighten it.** The reading comes from the desktop app, so no tool enforces this: write `Usage mode: <mode> (weekly N used, 5-hour N, read HH:MM)` at the top of NOW.md and in every release announcement, beside the three readings Bailey asked for. This replaces "ask above about 85 percent used".

**Calibrate the critic (adopted 2026-09-21).** `critic/calibration/cases.json` holds cases with a known answer: `should-fail` (a defect Bailey found on a build the critic had seen) and `should-pass` (something the critic or a judge wrongly flagged). Every case names its artefact and its source, and is confirmed against the files before it goes in; none is written from memory. Run the relevant reviewer over the cases when this rubric, a prompt in `critic/runner/` or the reviewing model changes, never per release, and record false accepts and false blocks in `critic/calibration/runs/`. Size the run and show Bailey the number before the first one. Every new owner-found defect adds a case, in the same retrospective that adds or strengthens its check.
...
| Stalled issues and repair counts | `stalledIssues` in `tools/critic-policy.mjs`, printed by `npm run critic:status`; `attempts` validated by `validateReport`. Information only |
| Usage modes, repair cycles per candidate, paper preflight | `usageModes` and `cadence` in policy.json hold the numbers; **a reviewer or driver applies them**, no tool can read the allowance |
