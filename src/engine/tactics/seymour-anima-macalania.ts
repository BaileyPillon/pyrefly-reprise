...
const GUARDIAN_IDS = ['guado-guardian-a', 'guado-guardian-b'] as const;
const ANIMA_ID = 'anima-macalania';

/** Mirrors `ai/macalania-rules.ts`; kept local so the tactic reads only state. */
const ELEMENT_CYCLE = ['ice', 'lightning', 'water', 'fire'] as const;
const NUL_FOR: Record<string, string> = {
  ice: 'NulFrost',
  lightning: 'NulShock',
  water: 'NulTide',
  fire: 'NulBlaze',
};
const NUL_STATUS: Record<string, string> = {
  ice: 'nulfrost',
  lightning: 'nulshock',
  water: 'nultide',
  fire: 'nulblaze',
 ...
 */
function gaugeOfId(engine: BattleEngine, id: CombatantId): number {
...
function act(engine: BattleEngine): 1 | 2 | 3 {
...
function unrobbedGuardian(engine: BattleEngine): CombatantId | undefined {
  const s = engine.state();
 ...
 * around here.
 */
function nulRow(commands: AvailableCommand[], engine: BattleEngine, element: string): Command | null {
...
function shellRow(commands: AvailableCommand[], engine: BattleEngine): Command | null {
 ...
 */
function bestSummon(commands: AvailableCommand[], engine: BattleEngine): Command | null {
 ...
 */
function aeonLine(commands: AvailableCommand[], engine: BattleEngine, actorId: CombatantId): Command | null {
