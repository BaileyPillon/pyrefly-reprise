 ...
 *
 * `intendedStrategy` asks here first. A tactic sees the live state and the
 * rows the engine offered — it never invents a command, only chooses among
 * legal ones — and returns `null` to fall through to the generic rules.
 *
 ...
 * auto-battler that "fixes" it would be encoding the failure mode the
 * encounter exists to teach.
 */
...
function row(commands: AvailableCommand[], labels: readonly string[], target?: CombatantId): AvailableCommand | undefined {
...
function aim(r: AvailableCommand, target?: CombatantId): Command {
...
function stacksOf(c: AnyCombatant, status: string): number {
...
function activeParty(engine: BattleEngine): AnyCombatant[] {
 ...
 * one.
 */
function revive(commands: AvailableCommand[], party: AnyCombatant[]): Command | null {
 ...
 *
 * **What this tactic does not achieve.** Against the shipped build it wins 44
 * of 96 seeds. Two defects outside this file cap it there and both are written
 * up in `tests/unit/strategy-chapter2.test.ts`: every party-switch row the
 * engine offers is disabled, so four of the seven members can never enter the
 * battle; and Confuse is never cleared by physical damage despite the status
 * data saying it is, so one Mind Blast that catches all three actives is
 * usually the run.
 */
const yunalesca: Tactic = (actorId, commands, engine) => {
  ...
  // NOTE — the free switch is **not** available in this build. `SwitchCommand`
  // hands the current turn to the incoming member at no cost (ffx-combat-core
  // §1.7), which would let the supporter's slot hold Yuna only on the turns
  // that need her and a striker (Wakka ~1,400 a swing against Yunalesca's
  // Defense, versus Yuna's ~170) on the rest. Every switch row the engine
  // offers is disabled: `commands.ts` gates it on `isAlive(bench)`, and
  // `isAlive` is `alive && !ko && onField`, which no reserve member can
  // satisfy. A tactic only picks rows the engine says are legal, so the line
  // is left un-played here and the defect is reported instead.
...
function hasAeonLeft(commands: AvailableCommand[]): boolean {
 ...
 */
function cheerUp(commands: AvailableCommand[], actor: AnyCombatant, living: AnyCombatant[]): Command | null {
...
/**
 * Swing instead of spending an Overdrive. Used by the supporter before Form III,
 * so Yuna's Grand Summon is still there when an aeon Overdrive is worth most
 * (§10.6: Grand Summon lets an aeon Overdrive on arrival, twice if its own
 * gauge was already full).
 */
function holdOverdrive(commands: AvailableCommand[], bossId: CombatantId): Command | null {
...
const TACTICS: ReadonlyArray<{ bossId: CombatantId; tactic: Tactic }> = [{ bossId: 'yunalesca', tactic: yunalesca }];
