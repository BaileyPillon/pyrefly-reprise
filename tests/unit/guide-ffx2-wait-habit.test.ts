...
describe('the rule itself', () => {
  ...
  it('only Chapters V and VI (and XV, unlisted, the ship brief) carry it; no FFX guide carries a clock rule', () => {
    const carriers = GUIDES.filter((g) => g.clockRules).map((g) => g.id);
    expect(carriers.sort()).toEqual([FFX2_LEBLANC_GUIDE.id, FFX2_VEGNAGUN_SHUYIN_GUIDE.id, 'ffx2-den-of-woe'].sort());
  });
});
